...
import {SimpleERC20} from "./day12-MyToken.sol";
contract PreorderTokens is SimpleERC20{
    ...
    event SaleFinalized(address indexed buyer,uint256 totalRaised, uint256 totalTokensSold);
    ...
    constructor(
        ...
        saleEndTime=block.timestamp+_salesDurationInSeconds;
    ...
    function finalizeSale()public payable {
        require(msg.sender==projectOwner,"Only owner can perform this function");
        require(!finalized,"Sale is already finalized");
        require(block.timestamp>saleEndTime,"Sale not finfshed yet");
        finalized=true;
        uint256 tokensSold=totalSupply-balanceOf[address(this)];
        (bool success,)=projectOwner.call{value:address(this).balance}("");
        require(success,"Transfer failed");
        emit SaleFinalized(projectOwner, totalRaised, tokensSold);
    }
    function timeRemaning()public view returns(uint256) {
