...
contract YieldFarming is ReentrancyGuard {
    ...
    // Added 'public' visibility to satisfy older linter rules
    constructor(
    ...
    function stake(uint256 amount) external nonReentrant {
        ...
        // Important: If this is a new staker, initialize their lastUpdate to now
    ...
    function updateRewards(address user) internal {
        ...
        if (staker.stakedAmount > 0) {
            // Solhint will still flag block.timestamp here; this is expected for DeFi
    ...
    function pendingRewards(address user) external view returns (uint256) {
        ...
        if (staker.stakedAmount > 0) {
            uint256 timeDiff = block.timestamp - staker.lastUpdate;
            uint256 rewardMultiplier = 10 ** uint256(stakingTokenDecimals);
            reward += (timeDiff * rewardRatePerSecond * staker.stakedAmount) / rewardMultiplier;
        }
