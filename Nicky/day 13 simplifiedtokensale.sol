...
contract SimplifiedTokenSale is SimpleERC20 {
    ...
    return (!finalized && block.timestamp >= saleStartTime && block.timestamp <= saleEndTime); //&&=and;!=not   
    }
    ...
    _transfer(address(this), msg.sender, tokenAmount); //address(this)= 合约地址
    emit TokensPurchased(msg.sender, msg.value, tokenAmount); //记录
    }

    function transfer(address _to, uint256 _value) public override returns (bool) {
    if (!finalized && msg.sender != address(this) && initialTransferDone) {
        require(false, "Tokens are locked until sale is finalized");//发售期间没有代币交易
        }
