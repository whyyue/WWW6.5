...
contract DecentralizedGovernance is ReentrancyGuard {
    ...
    uint256 public nextProposalId;//分配下一个ID
    uint256 public votingDuration;//提案开放投票时间
    uint256 public timelockDuration;//提案获胜后到执行前的等待期
    address public admin;
    uint256 public quorumPercentage;//代币供应量的最小百分比
    uint256 public proposalDepositAmount;//创建提案时必须锁定多少押金

    event ProposalCreated(uint256 indexed proposalId, address indexed proposer, string description);
    event Voted(uint256 indexed proposalId, address indexed voter, bool support, uint256 weight);
    event ProposalExecuted(uint256 indexed proposalId);//提案已执行
    event QuorumNotMet(uint256 indexed proposalId);
    event ProposalTimelockStarted(uint256 indexed proposalId);//提案赢得投票但在执行前进入时间锁延迟
    ...
    function executeProposal(uint256 proposalId) external {
        ...
        proposal.executed = true;
        _refundDeposit(proposalId);//退还押金。
