...
contract AutomatedMarketMaker is ERC20 {
    ...
    event LiquidityAdded(address indexed provider, uint256 amountA, uint256 amountB, uint256 liquidity);//有人添加代币到池子时触发
    event LiquidityRemoved(address indexed provider, uint256 amountA, uint256 amountB, uint256 liquidity);//有人移除池中流动性时触发
    event TokensSwapped(address indexed trader, address tokenIn, uint256 amountIn, address tokenOut, uint256 amountOut);//有人兑换 一种代币换另一种代币时触发
    ...
    function addLiquidity(uint256 amountA, uint256 amountB) external {
        ...
        uint256 liquidity;
        if (totalSupply() == 0) {
            liquidity = sqrt(amountA * amountB);//Uniswap常用的标准公式，用于公平地设定初始 LP 代币数量
        } else {
            liquidity = min(
    ...
    function removeLiquidity(uint256 liquidityToRemove) external returns (uint256 amountAOut, uint256 amountBOut) {
        ...
        amountAOut = liquidityToRemove * reserveA / totalLiquidity;//用户的 LP 份额 × 代币总储备 = 提取数量
    ...
    function swapAforB(uint256 amountAIn, uint256 minBOut) external {
        ...
        reserveA += amountAInWithFee;//手续费部分不参与下一笔交易的逻辑，留作奖励
        reserveB -= amountBOut;
    ...
    /// @dev Utility: Return the smaller of two values保持LP的平衡
    function min(uint256 a, uint256 b) internal pure returns (uint256) {
        return a < b ? a : b;
    }

    /// @dev Utility: Babylonian square root计算平方根
