...
contract PluginStore{ //插件商店，可以调用插件
    ...
    function registerPlugin(string memory key, address pluginAddress)external{
        plugins[key] = pluginAddress; //设置后通过key就能对应上到插件地址
    }

    function getPlugin(string memory key) external view returns(address){
        return plugins[key];  //用来得到插件地址的
    }
    ...
    (bool success, ) = plugin.call(data); //call是调用插件，但是数据存在本合约里
