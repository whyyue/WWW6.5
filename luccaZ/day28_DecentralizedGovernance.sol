...
/// @title Decentralized Governance System (ERC20-based)
/// @notice A DAO with weighted voting, quorum, proposal deposit, and time-locked execution
contract DecentralizedGovernance is ReentrancyGuard {
  ...
  event QuorumNotMet(uint256 id, uint256 votesTotal, uint256 quorumRequired);
  ...
  function createProposal(
    ...
    require(governanceToken.balanceOf(msg.sender) >= proposalDepositAmount, "Insufficient tokens for proposal deposit");
  ...
  function finalizeProposal(uint256 proposalId) external {
    //use storage to save gas since we will be modifying the proposal struct
    Proposal storage proposal = proposals[proposalId];
    require(block.timestamp >= proposal.deadline, "Voting period is still active");
    require(!proposal.executed, "Proposal already executed");
    require(proposal.executionTime == 0, "Proposal already finalized");
    ...
    if(totalVotes >= quorumRequired) {
      ...
      proposal.executed = true; // Mark as executed to prevent future execution attempts
      ...
      if (totalVotes < quorumRequired) {
        emit QuorumNotMet(proposalId, totalVotes, quorumRequired);
      }
    }
  }
  ...
  function executeProposal(uint256 proposalId) external nonReentrant {
    Proposal storage proposal = proposals[proposalId];
    require(!proposal.executed, "Proposal already executed");
    require(proposal.executionTime > 0 && block.timestamp >= proposal.executionTime, "Timelock not expired");
  ...
  function getProposalResults(uint256 proposalId) external view returns (string memory) {
    Proposal storage proposal = proposals[proposalId];
    require(block.timestamp >= proposal.deadline, "Voting period is still active");
