...
contract EnhancedSimpleEscrow {
    ...
    //immutalbe 不可变，可以在部署的时候传入，constant在代码便携阶段写死
    ...
    //solidity的任何只能合约都可以静默接收ETH，当合约接收ETH而没有人和数据时，会自动调用receive（）
    ...
    function raiseDispute() external {
        ...
        require(state == EscrowState.AWAITING_DELIVERY, " Can't disput now");

        state = EscrowState.DISPUTED;
        emit DisputeRaised(msg.sender);
    }

    function resolveDisput(bool _releaseToSeller) external {
    ...
    function cancelAfterTimeout() external {
        ...
        require(block.timestamp >= depositTime + deliveryTimeout, "Timeout not reaached");
