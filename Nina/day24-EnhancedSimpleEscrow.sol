...
contract EnhancedSimpleEscrow {
    ...
    // 阻止任何人试图发送的任何 ETH，而不调用正确的函数——比如 deposit()。当合约接收 ETH 而没有任何数据时，会自动调用这个特殊函数。
    ...
    // 资金全额退还给买家。—— 我们使用 `address(this).balance` 而不是 `amount`，只是为了额外安全——以防有任何意外进入合约（尽管通常只有 `amount` 会在那里）。
