...
contract NFTMarketplace is ReentrancyGuard {
    ...
    // transfer ---> call
    function Transfer2call(address payable to, uint256 value) internal {
    ...
    function buyNFT(address nftAddress, uint256 tokenId) external payable nonReentrant {
        ...
        // 市场费用
        if (feeAmount > 0) {
            Transfer2call(payable(feeRecipient),feeAmount);
        }

        // 创作者版税
        if (royaltyAmount > 0 && item.royaltyReceiver != address(0)) {

            Transfer2call(payable(item.royaltyReceiver),royaltyAmount);
        }

        // 卖家支付
        Transfer2call(payable(item.seller),sellerAmount);
